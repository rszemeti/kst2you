...
const loggerServerUrl = "https://europe-west2-kst-chat.cloudfunctions.net/kst-actions";
...
function addMapMarker(stn){
  ...
  var marker = new google.maps.Marker({position: stnLoc, map: map, title: stn.decoratedCallsign});
  if(stn.isAway()){
...
function logUser(userName) {
  setTimeout(() => {
    ...
    .then(data => {
      if (data.stats) {
        $('#loginCount').text(data.stats.totalLogins);
...
function drawMap(){
  ...
  var marker = new google.maps.Marker({position: qth, map: map, title: 'QTH'});
  addCircle(qth,100);
  addCircle(qth,200);
  addCircle(qth,300);
}
